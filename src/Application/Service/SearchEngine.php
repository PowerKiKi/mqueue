...
class SearchEngine
{
     ...
     */
    protected function getNovaCmd(): string
    {
        return 'python library/searchengine/nova3/nova2.py';
    }
     ...
     */
    protected function execute(string $cmd, int $timeout): string
    {
    ...
    public function parse(string $content): array
    {
        ...
        $duplicates = [];
        foreach (explode(PHP_EOL, mb_trim($content)) as $line) {
            $values = explode('|', mb_trim($line));
     ...
     */
    public function computeScores(string $title, array $sources): array
    {
        $rules = [
