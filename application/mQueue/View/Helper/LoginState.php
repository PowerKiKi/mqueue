    ...
    public function loginState()
    {
        ...
        if ($user) {
            $result .= '<a href="' . $this->view->serverUrl() . $this->view->url(
                ['controller' => 'user', 'action' => 'view', 'id' => $user->id],
                'singleid',
                true
            ) . '">' . $this->view->gravatar($user) . ' ' . $this->view->escape($user->nickname) . '</a> ';
            ...
            $result .= ' <a href="' . $this->view->serverUrl() . $this->view->url(
                ...
                'default',
                true
            ) . '">' . $this->view->translate('Subscribe') . '</a> ';

            $result .= '<a href="' . $this->view->serverUrl() . $this->view->url(
                ['controller' => 'user', 'action' => 'login'],
                'default',
                true
            ) . '">' . $this->view->translate('Login') . '</a>';
        }
